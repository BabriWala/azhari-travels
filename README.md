...
Imports accept both CRM contact exports and Meta/Facebook form response exports (`full_name`, `created_time`, `form_name`, `lead_status`, and `p:` phone prefixes). They match existing contacts by normalized phone, email, import identity or external lead ID. Bangladesh local `01…` and international `+8801…` numbers match, including Bengali digits. Store Excel phone cells as text to preserve leading zeros.
...
Each lead shows a Form responses section with readable question/answer text; original values remain stored. Global search includes questions and answers. Add response filters for exact answers, text contained in an answer, answered, or not answered, and combine several conditions with the existing owner/stage/source filters. All active conditions must match. Ad/campaign identifiers remain in Contact & import details instead of question filters.
