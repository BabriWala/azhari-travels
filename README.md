...
**Design studio** uses locally hosted Hind Siliguri and includes focused-card/editorial-split layouts, colors, corner radius, logo upload, and a live mobile/desktop design preview. Select Introduction, any step or Confirmation to manage its heading, supporting text, image, header/footer visibility, footer text and button labels. The confirmation button destination and brand link are configurable. Images use the existing compressed upload service. Save the campaign to publish design changes; questionnaires already in progress retain their snapshot.

**Answer → next step** supports ordered answer-matching rules and a default next step for each page. First match wins. Single choice, dropdown and yes/no rules can save and advance immediately when selected, provided the current step is valid. All destinations must be later steps; loops and skipping shared required contact/consent steps are rejected. Back follows the selected path. Skipped questions are excluded from validation, stored answers and qualification scores. Keep contact/consent in shared steps, and use default next-step controls to join alternative branches back together.

Superadmins can delete a campaign by confirming its URL slug. This immediately unpublishes it and moves it to **Deleted**; CRM leads, responses and history are preserved. Restoring returns it as an unpublished draft. Migration `000010_campaign_designer` adds the deletion marker.
