...
export type Campaign = { id?: string; slug: string; title: string; service: string; published: boolean; config: CampaignConfig; spend: number; currency: string; defaultOwner: string };
export default function CampaignAdmin() {
    ...
    return <main className="campaign-ui"><nav className="campaign-actions"><Link href="/admin/leads">← Lead workspace</Link><Link href="/admin/content">Content admin</Link></nav><h1>Campaigns & assessments</h1><p>Create a guided enquiry journey, then follow every response in your lead workspace.</p>
        {!ready ? <p>Loading…</p> : !token ? <section className="campaign-card"><p>Sign in with your admin account to continue.</p><Link href="/admin/leads">Sign in</Link></section> : <>
            ...
            {tab === "reports" ? <CampaignReports token={token} campaigns={campaigns} /> : edit && root ? <CampaignEditor key={edit.id || "new"} value={edit} people={people} busy={busy} onSave={save} onClose={() => setEdit(null)} /> : <div className="campaign-grid">{campaigns.map(c => <section className="campaign-card" key={c.id}><span className="campaign-eyebrow">{c.published ? "Published" : "Draft / paused"}</span><h2>{c.title}</h2><p>{c.service || "All services"} · {c.config.questions.filter(q => q.active).length} questions</p><p>{c.currency} {c.spend.toLocaleString()} campaign spend</p><div className="campaign-actions">{root && <button onClick={() => { setError(""); setEdit(c); }}>Edit campaign</button>}{c.published && <a href={`/campaign/${c.slug}`} target="_blank" rel="noopener noreferrer">Open landing page ↗</a>}</div><small>/campaign/{c.slug}</small></section>)}{!campaigns.length && <p>No campaigns yet. Create a draft to start building your questionnaire.</p>}</div>}
        </>}
    </main>;
}
