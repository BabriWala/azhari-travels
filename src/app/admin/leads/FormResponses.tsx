...
import { isWhatsappQuestion } from "../../lib/whatsapp";
...
export default function FormResponses({ extraFields }: { extraFields: string }) {
    ...
    return <section className="lw-form-responses" aria-label="Form responses">
        ...
        {entries.length ? <dl>{entries.map(entry => <div key={entry.key}><dt>{entry.question}</dt><dd className={entry.answer ? "" : "lw-muted"}>{entry.answer || "Not answered"}{isWhatsappQuestion(entry.question) && entry.answer && <> <WhatsAppLink phone={entry.answer} /></>}</dd></div>)}</dl>
