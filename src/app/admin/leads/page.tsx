...
import Followups, { Reminder } from "./Followups";
...
type Lead = { notes:string; reminders:Reminder[]; id: string; name: string; phone: string; email: string | null; status: string; owner: string; source: string; form: string; labels: string; secondaryPhone: string; whatsapp: string; importedCreated: string; createdAt: string; updatedAt: string; service: string; message: string; preferredContact: string; extraFields: string; _count: { conversations: number } };
...
export default function LeadsWorkspace() {
    ...
    const filtered = useMemo(() => leads.filter(l => (!reminderFilter || l.reminders.some(r=>reminderFilter === "completed" ? r.status === "completed" : r.status === "pending" && (reminderFilter === "pending" || (reminderFilter === "overdue" ? Date.parse(r.dueAt)<=Date.now() : new Date(r.dueAt).toDateString() === new Date().toDateString())))) && (!stage || l.status === stage) && (!owner || l.owner === owner) && (!source || l.source === source) && [l.name, l.phone, l.email, l.form, l.labels, l.owner, l.secondaryPhone, l.whatsapp, l.source, l.service, l.notes, ...l.reminders.map(r=>r.notes), searchableResponses(l.extraFields)].join(" ").toLowerCase().includes(readableResponse(search).toLowerCase()) && matchesResponses(l.extraFields, responseFilters, responseCombination)), [leads, search, stage, owner, source, responseFilters, responseCombination, reminderFilter]);
    useEffect(() => { setPage(1); setSelected([]); }, [search, stage, owner, source, responseFilters, responseCombination, pageSize, reminderFilter]);
    ...
    return <div className="lw">
        ...
        <div className="lw-table"><div className="lw-row lw-table-head">{isSuperadmin ? <input type="checkbox" aria-label="Select visible leads" checked={visible.length > 0 && visible.every(l => selected.includes(l.id))} onChange={e => setSelected(e.target.checked ? [...new Set([...selected, ...visible.map(l => l.id)])] : selected.filter(id => !visible.some(l => l.id === id)))} /> : <span/>}<span>NAME / CONTACT</span><span>STAGE</span><span>OWNER</span><span>SOURCE</span><span>ACTIVITY</span><span /></div>
        {visible.map(l => <div className={`lw-row ${activeId === l.id ? "lw-row-active" : ""}`} key={l.id}>{isSuperadmin ? <input type="checkbox" aria-label={`Select ${l.name}`} checked={selected.includes(l.id)} onChange={e => setSelected(e.target.checked ? [...selected, l.id] : selected.filter(id => id !== l.id))} /> : <span/>}<div className="lw-contact-cell"><button className="lw-person" onClick={() => chooseLead(l.id)}><span className="lw-avatar">{l.name.slice(0, 1)}</span><strong>{l.name}</strong></button><div className="lw-contact-number"><span>{l.phone || l.email || "No contact details"}</span>{l.phone && <WhatsAppLink phone={l.phone} />}</div>{responseEntries(l.extraFields).some(entry => entry.answer) && <small className="lw-response-badge">{responseEntries(l.extraFields).filter(entry => entry.answer).length} form answers</small>}</div><div><span className="lw-stage">{l.status}</span></div><div className="lw-owner">{l.owner === "Unassigned" ? <span className="lw-muted">Unassigned</span> : l.owner}</div><div className="lw-source">{l.source}<small title={l.form}>{l.form || l.service}</small></div><span className="lw-activity"><MessageSquare size={14} />{l._count.conversations}{l.reminders.some(r=>r.status === "pending") && <small className="lw-followup-badge">Next: {date(l.reminders.find(r=>r.status === "pending")!.dueAt)}</small>}</span><button className="lw-open" aria-label={`Open ${l.name}`} onClick={() => chooseLead(l.id)}><ChevronRight size={18} /></button></div>)}
