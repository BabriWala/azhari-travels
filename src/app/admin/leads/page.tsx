...
export default function LeadsWorkspace() {
    ...
    useEffect(() => {
        if (!activeId && !importOpen && !manage && !deleteTargets.length && !teamOpen && !helpOpen) return;
        ...
        const onKey = (event: KeyboardEvent) => {
            if (event.key === "Escape" && !interactionRef.current.busy && !interactionRef.current.recording) { setTeamOpen(false); setHelpOpen(false); setActiveId(""); setImportOpen(false); setManage(""); setDeleteTargets([]); }
            if (event.key === "Tab") {
    ...
    }, [activeId, importOpen, manage, deleteTargets.length, teamOpen, helpOpen]);
    ...
    const filtered = useMemo(() => leads.filter(l => (!stage || l.status === stage) && (!owner || l.owner === owner) && (!source || l.source === source) && [l.name, l.phone, l.email, l.form, l.labels, l.owner, l.secondaryPhone, l.whatsapp, l.source, l.service, searchableResponses(l.extraFields)].join(" ").toLowerCase().includes(readableResponse(search).toLowerCase()) && matchesResponses(l.extraFields, responseFilters, responseCombination)), [leads, search, stage, owner, source, responseFilters, responseCombination]);
    useEffect(() => { setPage(1); setSelected([]); }, [search, stage, owner, source, responseFilters, responseCombination, pageSize]);
    ...
    return <div className="lw">
        ...
        <div className="lw-container"><div className="lw-heading"><div><div className="lw-eyebrow">YOUR SALES COMMAND CENTRE</div><h1>Turn enquiries into<br />confirmed journeys<span>.</span></h1><p>One pipeline. A focused team. Every follow-up moves a lead forward.</p></div><div className="lw-heading-actions">{isSuperadmin && <button onClick={() => setTeamOpen(true)}><Users size={17} />Manage team</button>}<button onClick={() => setManage("stage")}><Plus size={17} />Stages</button><button className="lw-primary" onClick={() => { setImportOpen(true); setPreview(null); setFile(null); }}><Upload size={17} />Import leads</button></div></div>
        ...
        {visible.map(l => <div className={`lw-row ${activeId === l.id ? "lw-row-active" : ""}`} key={l.id}><input type="checkbox" aria-label={`Select ${l.name}`} checked={selected.includes(l.id)} onChange={e => setSelected(e.target.checked ? [...selected, l.id] : selected.filter(id => id !== l.id))} /><div className="lw-contact-cell"><button className="lw-person" onClick={() => chooseLead(l.id)}><span className="lw-avatar">{l.name.slice(0, 1)}</span><strong>{l.name}</strong></button><div className="lw-contact-number"><span>{l.phone || l.email || "No contact details"}</span>{l.phone && <WhatsAppLink phone={l.phone} />}</div>{responseEntries(l.extraFields).some(entry => entry.answer) && <small className="lw-response-badge">{responseEntries(l.extraFields).filter(entry => entry.answer).length} form answers</small>}</div><div><span className="lw-stage">{l.status}</span></div><div className="lw-owner">{l.owner === "Unassigned" ? <span className="lw-muted">Unassigned</span> : l.owner}</div><div className="lw-source">{l.source}<small title={l.form}>{l.form || l.service}</small></div><span className="lw-activity"><MessageSquare size={14} />{l._count.conversations}</span><button className="lw-open" aria-label={`Open ${l.name}`} onClick={() => chooseLead(l.id)}><ChevronRight size={18} /></button></div>)}
