...
import { dailyStatus, dailyWorkLabels, DailyWorkRecord, DailyWorkStatus, workDay } from "../../lib/leadDailyWork";
...
type Lead = { assessments: { score: number; qualification: string; completedAt: string | null; campaign: { title: string } }[]; dailyWork:DailyWorkRecord[]; uploads:UploadBatch[]; conversations:LeadNote[]; notes:string; reminders:Reminder[]; id: string; name: string; phone: string; email: string | null; status: string; owner: string; source: string; form: string; labels: string; secondaryPhone: string; whatsapp: string; importedCreated: string; createdAt: string; updatedAt: string; service: string; message: string; preferredContact: string; extraFields: string; _count: { conversations: number } };
...
export default function LeadsWorkspace() {
    const [today,setToday]=useState(()=>workDay()),[dailyFilter,setDailyFilter]=useState("");
    useEffect(()=>{const timer=setInterval(()=>setToday(workDay()),1000);return ()=>clearInterval(timer);},[]);
    ...
    const openedLinkedLead = useRef(false);
    useEffect(() => {
        const id = new URLSearchParams(window.location.search).get("lead");
        if (!openedLinkedLead.current && id && leads.some(l => l.id === id)) { openedLinkedLead.current = true; setActiveId(id); }
    }, [leads]);
    ...
    async function saveDailyWork(id:string,status:DailyWorkStatus,day:string) {
        const record=await api("/"+id+"/daily-work",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({status,day})});
        setLeads(previous=>previous.map(lead=>lead.id===id?{...lead,dailyWork:[record]}:lead));
        if(activeId===id)void loadMessages(id).catch(()=>{});
    }
    const filtered = useMemo(() => leads.filter(l => (!dailyFilter || dailyStatus(l.dailyWork,today)===dailyFilter) && (!uploadDate || l.uploads.some(b=>uploadDay(b.createdAt)===uploadDate)) && (!uploadBatch || l.uploads.some(b=>String(b.id)===uploadBatch)) && (!reminderFilter || l.reminders.some(r=>reminderFilter === "completed" ? r.status === "completed" : r.status === "pending" && (reminderFilter === "pending" || (reminderFilter === "overdue" ? Date.parse(r.dueAt)<=Date.now() : new Date(r.dueAt).toDateString() === new Date().toDateString())))) && (!stage || l.status === stage) && (!owner || l.owner === owner) && (!source || l.source === source) && [l.name, l.phone, l.email, l.form, l.labels, l.owner, l.secondaryPhone, l.whatsapp, l.source, l.service, l.notes, ...l.uploads.map(b=>b.name||b.filename), ...l.conversations.map(n=>n.text), ...l.reminders.map(r=>r.notes), searchableResponses(l.extraFields)].join(" ").toLowerCase().includes(readableResponse(debouncedSearch).toLowerCase()) && matchesResponses(l.extraFields, debouncedResponseFilters, responseCombination)), [leads, today, dailyFilter, uploadDate, uploadBatch, debouncedSearch, stage, owner, source, debouncedResponseFilters, responseCombination, reminderFilter]);
    useEffect(() => { setPage(1); setSelected([]); }, [today, dailyFilter, stage, uploadDate, uploadBatch, debouncedSearch, owner, source, debouncedResponseFilters, responseCombination, pageSize, reminderFilter]);
    ...
    return <div className="lw">
        <header className="lw-top"><Link href="/admin/leads" className="lw-brand"><img src="/Logo.png" alt="" />AZHARI <span>LEADS</span></Link><div className="lw-top-actions"><span className="lw-live">{user?.name} · {isSuperadmin ? "Superadmin" : "Subadmin"}</span><Link href="/admin/campaigns">Campaigns & assessments</Link><button onClick={()=>setHelpOpen(true)}>How CRM works</button>{isSuperadmin && <Link href="/admin/content">Content admin <ArrowUpRight size={14} /></Link>}<button aria-label="Sign out" onClick={() => { void fetch("/api/admin/logout",{method:"POST",headers:{Authorization:`Bearer ${token}`}}); localStorage.removeItem("azhari_admin_token"); setUser(null); setToken(""); setLeads([]); setActiveId(""); }}><LogOut size={18} /></button></div></header>
        <div className="lw-container"><div className="lw-heading"><div><div className="lw-eyebrow">LEAD MANAGEMENT</div><h1>Leads workspace</h1><p>Manage contacts, daily work and follow-ups in one place.</p></div><div className="lw-heading-actions"><button className="lw-primary" onClick={()=>setAddLeadOpen(true)}><Plus size={17}/>Add lead</button><button onClick={()=>setTemplatesOpen(true)}><MessageSquare size={17}/>Templates</button>{isSuperadmin && <button onClick={() => setTeamOpen(true)}><Users size={17} />Manage team</button>}<button onClick={() => setManage("stage")}><Plus size={17} />Stages</button><button className="lw-primary" onClick={() => { setImportOpen(true); }}><Upload size={17} />Import leads</button></div></div>
        ...
        <section className="lw-daily-dashboard" aria-label="Today’s lead work"><h2>Today’s work · {today}</h2><p>Shared team status for each lead. Starts fresh daily at midnight in Dhaka; previous updates remain in conversation history.</p><div className="lw-inline-actions"><button aria-pressed={!dailyFilter} onClick={()=>setDailyFilter("")}>All leads</button>{Object.entries(dailyWorkLabels).map(([value,label])=><button key={value} aria-pressed={dailyFilter===value} onClick={()=>setDailyFilter(dailyFilter===value?"":value)}>{label} ({leads.filter(l=>dailyStatus(l.dailyWork,today)===value).length})</button>)}</div></section>
        ...
        {importOpen&&<ImportLeads api={api} onClose={()=>setImportOpen(false)} onSaved={async()=>{await refresh();setNotice("Named upload saved.");}}/>}
