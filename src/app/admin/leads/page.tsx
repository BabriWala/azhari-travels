...
export default function LeadsWorkspace() {
    ...
    const filtered = useMemo(() => leads.filter(l => (!stage || l.status === stage) && (!owner || l.owner === owner) && (!source || l.source === source) && [l.name, l.phone, l.email, l.form, l.labels, l.owner, l.secondaryPhone, l.whatsapp, l.source, l.service, searchableResponses(l.extraFields)].join(" ").toLowerCase().includes(readableResponse(search).toLowerCase()) && matchesResponses(l.extraFields, responseFilters)), [leads, search, stage, owner, source, responseFilters]);
    useEffect(() => { setPage(1); setSelected([]); }, [search, stage, owner, source, responseFilters]);
