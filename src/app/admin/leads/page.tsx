...
export default function LeadsWorkspace() {
    ...
    useEffect(() => {
        if (!activeId && !importOpen && !manage && !deleteTargets.length) return;
        ...
        const onKey = (event: KeyboardEvent) => {
            if (event.key === "Escape" && !busy && !recording) { setActiveId(""); setImportOpen(false); setManage(""); setDeleteTargets([]); }
            if (event.key === "Tab") {
    ...
    }, [activeId, importOpen, manage, busy, recording, deleteTargets.length]);
    ...
    async function deleteLeads(ids: string[], confirmation: string) {
        setBusy(true);
        try {
            const result = await api("", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ids, confirmation, confirmations: 3 }) });
