...
export default function LeadsWorkspace() {
    ...
    const filtered = useMemo(() => leads.filter(l => (!dailyFilter || dailyStatus(l.dailyWork,today)===dailyFilter) && (!uploadDate || l.uploads.some(b=>uploadDay(b.createdAt)===uploadDate)) && (!uploadBatch || l.uploads.some(b=>String(b.id)===uploadBatch)) && (!reminderFilter || l.reminders.some(r=>reminderFilter === "completed" ? r.status === "completed" : r.status === "pending" && (reminderFilter === "pending" || (reminderFilter === "overdue" ? Date.parse(r.dueAt)<=Date.now() : new Date(r.dueAt).toDateString() === new Date().toDateString())))) && (!stage || l.status === stage) && (!owner || l.owner === owner) && (!source || l.source === source) && [l.name, l.phone, l.email, l.form, l.labels, l.owner, l.secondaryPhone, l.whatsapp, l.source, l.service, l.notes, ...l.conversations.map(n=>n.text), ...l.reminders.map(r=>r.notes), searchableResponses(l.extraFields)].join(" ").toLowerCase().includes(readableResponse(debouncedSearch).toLowerCase()) && matchesResponses(l.extraFields, debouncedResponseFilters, responseCombination)), [leads, today, dailyFilter, uploadDate, uploadBatch, debouncedSearch, stage, owner, source, debouncedResponseFilters, responseCombination, reminderFilter]);
