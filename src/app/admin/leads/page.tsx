...
type Lead = { conversations:LeadNote[]; notes:string; reminders:Reminder[]; id: string; name: string; phone: string; email: string | null; status: string; owner: string; source: string; form: string; labels: string; secondaryPhone: string; whatsapp: string; importedCreated: string; createdAt: string; updatedAt: string; service: string; message: string; preferredContact: string; extraFields: string; _count: { conversations: number } };
...
export default function LeadsWorkspace() {
    ...
    const filtered = useMemo(() => leads.filter(l => (!reminderFilter || l.reminders.some(r=>reminderFilter === "completed" ? r.status === "completed" : r.status === "pending" && (reminderFilter === "pending" || (reminderFilter === "overdue" ? Date.parse(r.dueAt)<=Date.now() : new Date(r.dueAt).toDateString() === new Date().toDateString())))) && (!stage || l.status === stage) && (!owner || l.owner === owner) && (!source || l.source === source) && [l.name, l.phone, l.email, l.form, l.labels, l.owner, l.secondaryPhone, l.whatsapp, l.source, l.service, l.notes, ...l.conversations.map(n=>n.text), ...l.reminders.map(r=>r.notes), searchableResponses(l.extraFields)].join(" ").toLowerCase().includes(readableResponse(debouncedSearch).toLowerCase()) && matchesResponses(l.extraFields, debouncedResponseFilters, responseCombination)), [leads, debouncedSearch, stage, owner, source, debouncedResponseFilters, responseCombination, reminderFilter]);
    useEffect(() => { setPage(1); setSelected([]); }, [debouncedSearch, stage, owner, source, debouncedResponseFilters, responseCombination, pageSize, reminderFilter]);
